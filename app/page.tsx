...
// បញ្ជីឈ្មោះនិស្សិត ITE A3 (ភួងផល សំណាង និង ម៉ក់ លីឈុន នៅលេខរៀងទី ១ និង ទី ២)
const STUDENT_LIST = [
...
export default function Home() {
  ...
  // ប្រព័ន្ធគ្រប់គ្រង Logic កាលវិភាគ
  const generateSchedule = (studentList: Student[]) => {
    ...
    // កំណត់រក Index នៃថ្ងៃនេះក្នុង Loop (0 = ចន្ទ, 1 = អង្គារ, 2 = ពុធ, 3 = ព្រហស្បតិ៍, 4 = សុក្រ)
    const todayInLoopIdx = (currentDayNum >= 1 && currentDayNum <= 5) ? currentDayNum - 1 : 0;
    ...
    for (let d = 0; d < 20; d++) {
      ...
      // កែសម្រួល Logic ត្រង់នេះ៖ ធានាថាថ្ងៃនេះនឹងទទួលបាន baseIdx = 0 (ចាប់ផ្តើមពី ភួងផល សំណាង)
      const diffFromToday = d - todayInLoopIdx;
      const baseIdx = ((diffFromToday * 2) % len + len) % len;
